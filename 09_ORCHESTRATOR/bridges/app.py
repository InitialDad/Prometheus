...
from . import plan_room, gpt_bridge
...
# ---- plan room (dual-LLM: Fable + GPT) ----------------------------------

@app.route("/api/planroom/status")
def api_planroom_status():
    """Which models are wired up + the role→backend routing."""
    return jsonify({
        "fable": plan_room.CLAUDE_EXE.exists(),
        "gpt": gpt_bridge.available(),
        "gpt_model": gpt_bridge.DEFAULT_MODEL,
        "roles": plan_room.ROLE_DEFAULT_BACKEND,
        "role_labels": plan_room.ROLE_LABEL,
    })


@app.route("/api/planroom/start", methods=["POST"])
def api_planroom_start():
    body = request.get_json(force=True, silent=True) or {}
    serial = body.get("serial")
    ...
    cfg = GAME_PROJECTS.get(serial) if serial else None
    project_dir = cfg.get("project_dir") if cfg else None
    # Optional overrides: role_backend {fable|sol|luna: fable|gpt}, round caps.
    role_backend = dict(plan_room.ROLE_DEFAULT_BACKEND)
    for role, backend in (body.get("role_backend") or {}).items():
        if role in role_backend and backend in plan_room.BACKENDS:
            role_backend[role] = backend
    spec = plan_room.PlanSpec(
        serial=serial, goal=goal, project_dir=project_dir,
        role_backend=role_backend,
        max_plan_rounds=int(body.get("max_plan_rounds", 3)),
        max_code_rounds=int(body.get("max_code_rounds", 3)),
    )
    sid = plan_room.start_session(spec)
    return jsonify({"ok": True, "session_id": sid})


@app.route("/api/planroom/sessions")
def api_planroom_sessions():
    return jsonify({"sessions": plan_room.list_sessions(
        limit=int(request.args.get("limit", 30)))})


@app.route("/api/planroom/sessions/<int:sid>")
def api_planroom_session(sid):
    row = plan_room.get_session(sid)
    if not row:
        abort(404)
    return jsonify(row)
