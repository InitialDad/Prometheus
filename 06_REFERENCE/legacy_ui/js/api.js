...
export const api = {
  ...
  // ---- plan room (Fable + GPT) ----
  prStatus:     ()           => J('/api/planroom/status'),
  prStart:      (body)       => J('/api/planroom/start', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(body) }),
  prSessions:   ()           => J('/api/planroom/sessions'),
  prSession:    (id)         => J(`/api/planroom/sessions/${id}`),
  prImplement:  (id, implementer)=> J(`/api/planroom/sessions/${id}/implement`, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ implementer }) }),
};
